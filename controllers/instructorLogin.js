...
const signup = async (req, res) => {
  try {
    const { id, name, email, role, faculty, password } = req.body;
    if (!id || !name || !email || !role || !faculty || !password) {
    ...
    const query = `INSERT INTO instructor
                 (id, name, email, role, faculty, password) VALUES
                (?, ?, ?, ?, ?, ?)`;
    connection.query(query, [id, name, email, role, faculty, hash]);
